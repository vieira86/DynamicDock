...
class DockingHandler:
    ...
    def run_docking(
        ...
        try:
            ...
            return {
                "success": False,
                "error": (
                    f"Could not run the AutoDock Vina executable "
                    f"('{self.vina_executable}'). Run 'python setup.py' from the "
                    f"project root to download it automatically for your OS, or "
                    f"set the VINA_EXECUTABLE environment variable."
                ),
                "output": "",
            }
    ...
    def save_docked_complex(self, receptor_path: str, docked_ligand_path: str, output_path: str = None) -> str:
        ...
        try:
            ...
            obabel_executable = paths.find_obabel_executable() or "obabel"
            ...
            if receptor_path.endswith('.pdbqt'):
                ...
                cmd = [obabel_executable, receptor_path, '-O', receptor_pdb]
                subprocess.run(cmd, check=True, capture_output=True, text=True)

            # Convert docked ligand to PDB
            ligand_pdb = docked_ligand_path.replace('.pdbqt', '_ligand.pdb')
            cmd = [obabel_executable, docked_ligand_path, '-O', ligand_pdb]
            subprocess.run(cmd, check=True, capture_output=True, text=True)
